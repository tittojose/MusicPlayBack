 ...
 *
 */
public class ProgressWheelIndicator extends View {
	 ...
	 * @param attrs
	 */
	public ProgressWheelIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public void incrementProgress(int f) {
		isSpinning = false;
		progress = f;
		int progress_per=((f*100)/360);
		if (progress > 360)
			progress = 0;
		//setText(Math.round(((float)progress/360)*100) + "%");
		//spinHandler.sendEmptyMessage(0);
